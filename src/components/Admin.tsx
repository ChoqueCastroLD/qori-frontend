...
export default function Admin() {
  ...
  const [tab, setTab] = useState<"metrics" | "growth" | "purchases" | "recharges" | "affiliates" | "users" | "raffles" | "winners" | "create">("metrics");
  ...
  async function createAffiliate(code: string, name: string, note: string) {
    ...
    else setMsg(`Error: ${res.data?.error === "code_taken" ? "ese código ya existe" : res.data?.error === "invalid_code" ? "código inválido (minúsculas, números y guiones)" : res.data?.error ?? "no se pudo"}`);
    return res.ok;
  }
  async function patchAffiliate(id: string, patch: any) {
    const res = await adminFetch(`/admin/affiliates/${id}`, { method: "PATCH", body: JSON.stringify(patch) });
    if (res.ok) reloadAffiliates(); else setMsg(`Error: ${res.data?.error ?? "no se pudo"}`);
  }
  async function payAffiliate(id: string, usd: number) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-10">
      ...
      <div className="mt-5 flex gap-1 overflow-x-auto border-b border-slate-200">
        {([["metrics", "Métricas"], ["growth", "Crecimiento"], ["purchases", "Compras"], ["recharges", pendingTopups && pendingTopups.length ? `Recargas (${pendingTopups.length})` : "Recargas"], ["affiliates", "Afiliados"], ["users", "Usuarios"], ["raffles", "Sorteos"], ["winners", "Ganadores"], ["create", "Crear sorteo"]] as const).map(([k, l]) => (
...
function Affiliates({ affiliates, stats, onCreate, onPatch, onPay }: { affiliates: any[] | null; stats: any; onCreate: (c: string, n: string, note: string) => Promise<boolean>; onPatch: (id: string, patch: any) => void; onPay: (id: string, usd: number) => void }) {
  ...
  if (!affiliates) return <p className="mt-6 text-slate-400">Cargando afiliados…</p>;
  const usd = (c: number) => `$${((c || 0) / 100).toFixed(2)}`;
  const origin = typeof location !== "undefined" ? location.origin : "https://qori.cc";
  ...
  return (
    <div className="mt-6 space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <form
          onSubmit={async (e) => { e.preventDefault(); if (!code.trim() || !name.trim()) return; setCreating(true); const ok = await onCreate(code.trim().toLowerCase(), name.trim(), note.trim()); setCreating(false); if (ok) { setCode(""); setName(""); setNote(""); } }}
          className="mt-3 grid gap-2 sm:grid-cols-3"
        >
          <div><label className="text-xs text-slate-500">Código (ej. totoroscan)</label><input value={code} onChange={(e) => setCode(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))} className={`${inp} font-mono`} placeholder="totoroscan" /></div>
          <div><label className="text-xs text-slate-500">Nombre del scan</label><input value={name} onChange={(e) => setName(e.target.value)} className={inp} placeholder="Totoro Scans" /></div>
          <div><label className="text-xs text-slate-500">Nota / contacto (opcional)</label><input value={note} onChange={(e) => setNote(e.target.value)} className={inp} placeholder="Discord: totoro#1234" /></div>
          <div className="sm:col-span-3"><button disabled={creating || !code.trim() || !name.trim()} className="rounded-lg bg-slate-900 px-5 py-2 text-sm font-semibold text-white disabled:bg-slate-400">{creating ? "Creando…" : "Crear afiliado"}</button></div>
        </form>
      </div>

      {affiliates.length === 0 ? (
        <p className="rounded-xl border border-dashed border-slate-200 p-10 text-center text-slate-400">Aún no hay afiliados. Crea el primero arriba.</p>
      ) : (
        <div className="space-y-3">{affiliates.map((a) => <AffiliateRow key={a.id} a={a} origin={origin} usd={usd} onPatch={onPatch} onPay={onPay} />)}</div>
      )}
    </div>
  );
}

function AffiliateRow({ a, origin, usd, onPatch, onPay }: any) {
  ...
  async function toggle() {
    ...
    if (n && !detail) { const d = await adminFetch(`/admin/affiliates/${a.id}`); if (d.ok) setDetail(d.data); }
  }
  return (
    <div className={`rounded-xl border bg-white ${a.active ? "border-slate-200" : "border-slate-200 opacity-60"}`}>
      <div className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div className="min-w-0">
          ...
          <button
            ...
            className="mt-1 inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2 py-1 font-mono text-xs text-slate-600 hover:bg-slate-200"
            title="Copiar link"
          >
            <Icon name={copied ? "check" : "copy"} className="h-3.5 w-3.5" /> {link}
          </button>
          {a.note && <div className="mt-1 text-xs text-slate-400">{a.note}</div>}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 p-3">
        <input value={pay} onChange={(e) => setPay(e.target.value)} type="number" step="0.5" placeholder="USD pagado" className="w-32 rounded-lg border border-slate-200 px-2.5 py-1.5 text-sm" />
        <button onClick={() => { const v = Number(pay); if (v > 0) { onPay(a.id, v); setPay(""); } }} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-emerald-500">Registrar pago</button>
        <button onClick={toggle} className="ml-auto inline-flex items-center gap-1 text-sm font-semibold text-slate-600 hover:text-slate-900">{open ? "Ocultar" : "Ver referidos"} <Icon name={open ? "chevron-up" : "chevron-down"} className="h-4 w-4" /></button>
      </div>

      {open && (
        <div className="border-t border-slate-100 p-3">
          {!detail ? <Skeleton className="h-16 w-full rounded-lg" /> : detail.users.length === 0 ? (
            <p className="text-sm text-slate-400">Nadie ha entrado con este código todavía.</p>
          ) : (
            <table className="w-full text-left text-sm">
              <thead className="text-xs text-slate-400"><tr><th className="py-1.5 pr-3">Usuario</th><th className="py-1.5 pr-3">Se unió</th><th className="py-1.5 pr-3">Gastó</th><th className="py-1.5">Válido</th></tr></thead>
              <tbody>
                {detail.users.map((u: any, i: number) => (
                  <tr key={i} className="border-t border-slate-100">
                    <td className="py-2 pr-3"><div className="font-medium text-slate-800">{u.nickname || "-"}</div><div className="text-xs text-slate-400">{u.email}</div></td>
                    <td className="py-2 pr-3 text-xs text-slate-500">{fmt(u.createdAt)}</td>
                    <td className="py-2 pr-3 text-slate-600">{usd(u.spentUsdCents)}</td>
                    <td className="py-2">{u.valid ? <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700">Sí</span> : <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500">No</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>

...
export default function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  const [ref, setRef] = useState(
    typeof window !== "undefined"
      ? ((new URLSearchParams(location.search).get("ref") ?? "").toUpperCase() || storedRef())
      : "",
  );
  ...
  const googleBtn = (
    <a href={ref ? `/api/auth/google?ref=${encodeURIComponent(ref)}` : "/api/auth/google"} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">

...
// Public referral-code lookup: type a code, see how many it referred and who.
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <form onSubmit={search} className="mt-3 flex gap-2">
        <input
          ...
          placeholder="Ej: breakscan o D8M7L4DW"
          className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"
        />
        <button disabled={loading || !code.trim()} className="shrink-0 rounded-lg bg-slate-900 px-5 py-2 text-sm font-semibold text-white hover:bg-slate-700 disabled:bg-slate-400">
          ...
          <div className="mt-4">
            <div className="flex items-center gap-3 rounded-xl bg-slate-50 p-3">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-emerald-700">{nf(res.count)}</div>
                <div className="text-xs text-slate-500">referido{res.count === 1 ? "" : "s"}</div>
              </div>
            </div>

            {res.count > 0 && (
              <div className="mt-3">
                <div className="mb-2 text-xs font-semibold text-slate-500">Quiénes entraron con este código</div>
                <ul className="grid gap-2 sm:grid-cols-2">
                  {res.referred.map((u: any, i: number) => {
                    const inner = (
                      <>
                        <Avatar url={u.avatarUrl} name={u.nickname || u.username || "?"} />
